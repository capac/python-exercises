...
# each element of array B is an integer that can have one of
...
#         if (A[index-1] > A[index] & (B[index-1] == 1 & B[index] == 0)):
#             number_fish -= 1
#     return number_fish


def solution(A, B):
    stack = []  # Stack to track downstream fish
    survivors = 0  # Count of surviving fish
    ...
    for i in range(len(A)):
        if B[i] == 1:  # Downstream fish
            ...
            while stack:
                if stack[-1] > A[i]:  # Downstream fish wins
                    break
                else:  # Upstream fish eats downstream fish
                    stack.pop()
            else:  # If stack is empty, upstream fish survives
                survivors += 1
    # Upstream survivors + remainingdownstream fish
