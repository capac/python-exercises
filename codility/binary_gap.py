...
# its longest binary gap. The function should return 0 if N doesn't contain
# a binary gap.
...
def solution(n):
    bin_n = bin(n)[2:]
    gaps = bin_n.strip('0').split('1')
    return bin_n, list(map(len, gaps))


if __name__ == '__main__':
    for n in [1041, 32, 15, 529]:
        n0, n1 = solution(n)
        print(f'{n} ({n0}): {n1}')
